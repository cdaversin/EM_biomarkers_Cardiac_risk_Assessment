...
outdir = here / "results"
#statefile = sys.argv[3] / "results/state.json"
#T=72000
T=72000
save_freq=1
...
if config.load_state:
    ...
    coupling = EMCoupling.from_state(
        #path = "/cluster/projects/nn9249k/lprisai/60b_Tor_geo10x2x2_pcl1000_ICcell_wave/results/state.h5",
        path=outdir / "state.h5",
        drug_factors_file=config.drug_factors_file,
        popu_factors_file=config.popu_factors_file,
        disease_state=config.disease_state,
        PCL=config.PCL,
    ...
    logger.info("Model for coupling is: " + str(config.coupling_type))
else:
...
runner = simcardems.Runner.from_models(config=config, coupling=coupling)
logger.info("I have configure the runner")
...
logger.info("I have solve the simulation")
